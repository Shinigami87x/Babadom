  ...
  return (
    <div className="w-1/4 bg-white fixed right-0 top-0 z-50" ref={cartRef}>
      <div className="relative h-screen w-full bg-white float-right px-10 py-2.5 border-4 border-l-black">
        <button className="flex items-center text-lg font-medium cursor-pointer gap-0.5 border-0 bg-transparent" type="button" onClick={() => setShowCart(false)}>
        ...
        {cartItems.length < 1 && (
          <div className="m-10 text-center">
            ...
            <Link href="/" passHref>
              <button className="w-full max-w-sm px-2.5 py-3 rounded-2xl border-0 text-base mt-2.5  bg-red-600 text-white cursor-pointer scale-100 ease-in duration-500 hover:scale-110" type="button" onClick={() => setShowCart(false)}>
                Continue Shopping
              </button>
            </Link>
          </div>
          ...
          {cartItems.length >= 1 &&
            cartItems.map((item) => (
              <div className="flex py-1" key={item._id}>
                <img className="w-1/4 h-1/4 rounded" src={urlFor(item?.image[0])} />
                  ...
                  <div className="relative flex flex-row justify-between w-full">
                    <div>
                      <p className="flex flex-row ml-2 mt-0.5">
                        <span className="text-base pt-2 px-2 cursor-pointer border-2 rounded-full border-black" onClick={() => toggleCartItemQuantity(item._id, 'dec') }>
                          <AiOutlineMinus />
                        </span>
                        <span className="text-base px-2 py-1 pt-2 cursor-pointer" onClick="">{item.quantity}</span>
                        <span className="text-base pt-2 px-2 cursor-pointer border-2 rounded-full border-black" onClick={() => toggleCartItemQuantity(item._id, 'inc') }>
                          <AiOutlinePlus />
                        </span>
                      </p>
                    </div>
